...
<template>
    <div class="relative" @click="clicked" ref="item">
        ...
        ]" aria-expanded="false" href="#" @click.prevent="clickLink">
            {{ item.title }}
            <svg v-if="item.items.length" class="h-5 w-5 flex-none text-gray-400 ml-auto" viewBox="0 0 20 20"
        ...
        <div v-if="item.items.length && visible" :class="[
            ...
            <div class="p-4">
                <div v-for="(opItem, index) in item.items.filter(
                    (x) => x.visible
                )" :key="index"
                    ...
                    <div class="flex-auto w-full">
                        <a :href="opItem.route" class="m-0 block font-semibold text-gray-900 px-11 dark:text-white">
                            {{ opItem.title }}
                        </a>
                    </div>
                </div>
